...
' Sets Rights on a Sub-Collection in SCCM 2007
' v1.4 7/16/2008
' Emmanuel Tsouris
'
'Example:  (be sure to use quotes around the group name)
' cscript setSubCollectionRights.vbs /server:serverName /site:ABC /col:ABC011AB /user:"domain\activeDirectoryGroupNameOrUser" /rights:R
' cscript setSubCollectionRightBits.vbs /server:serverName /site:ABC /col:ABC011AB /user:"domain\activeDirectoryGroupNameOrUser" /rights:full-DR
'
' /rights:WIPE
...
Sub CopyRightsToSubCollection(targetParentCollectionID, targetUserName) 
    WScript.Echo Now() & vbTab & "Parent Collection ID: " & targetParentCollectionID
    ...
    'WScript.Echo TypeName(subCollections)
    WScript.Echo "Count of collections "
    WScript.Echo subCollections.Count

	'Loop each sub-collection
    For each subCollection in subCollections
		
        Wscript.Echo subCollection.SubCollectionID
